...
# Empfänger-Konfiguration (wie gewünscht hinterlegt)
RECIPIENTS = ["Kevin.woelki@augsburg.de", "kevinworlki@outlook.de"]

st.set_page_config(page_title="KOD Augsburg - Security Portal", page_icon="🛡️", layout="wide") 

# --- 2. CSS STYLING (VERBESSERT) ---
st.markdown("""
    <style>
    .report-card { 
    ...
    with col2:
        ...
        st.markdown("""
            <div style="text-align: center;">
                <h1 style="color: #004b95; margin-bottom: 0;">🛡️ KOD Sicherheitsbereich</h1>
                <p style="color: #666; font-weight: bold;">Stadt Augsburg - Ordnungsamt</p>
                <hr>
            </div>
        """, unsafe_allow_html=True)
        
        with st.container():
            pw_input = st.text_input("Bitte Dienst-Passwort eingeben", type="password", placeholder="🔒 Passwort...")
            if st.button("🚪 System betreten", use_container_width=True):
        ...
        st.markdown("<p style='text-align: center; font-size: 0.8em; color: gray;'>Nur für autorisiertes Personal</p>", unsafe_allow_html=True)
    st.stop() 

# --- 6. HAUPT-APP (BERICHTE) ---
...
# --- SIDEBAR (ADMIN-LOGIN) ---
with st.sidebar:
    st.markdown("### 🛠️ System-Verwaltung")
    # Hier bleibt die Anmeldung bestehen, aber ohne "Gefahr", dass jemand den Code sieht
    with st.expander("🔑 Admin-Login"):
        admin_pw_input = st.text_input("Passwort", type="password", key="admin_key_input")
        if admin_pw_input == ADMIN_PW:
            st.session_state["admin_auth"] = True
            st.success("Admin-Status: AKTIV")
        else:
            st.session_state["admin_auth"] = False
            if admin_pw_input: st.error("Falsches Passwort")
