...
st.markdown("""
    <style>
    .report-card { background-color: #f8f9fa; border-radius: 10px; padding: 15px; border-left: 5px solid #004b95; margin-bottom: 10px; border: 1px solid #e0e0e0; }
    .delete-btn { color: #ff4b4b; }
    </style>
    """, unsafe_allow_html=True)

# --- 3. SICHERHEIT & SESSION STATE ---
...
ADMIN_PW = "admin789" # Passwort für Löschfunktion
...
STRASSEN_AUGSBURG = sorted([
    ...
    "Rathausplatz", "Königsplatz", "Maximilianstraße", "Bahnhofstraße", "Moritzplatz", 
    "Ulmer Straße", "Donauwörther Straße", "Friedberger Straße", "Haunstetter Straße", "Zugspitzstraße"
    # ... (weitere Straßen hier einfügen)
])

FESTSTELLUNGEN = sorted(["§ 111 OWiG", "§ 118 OWiG Belästigung", "Alkohol Spielplatz", "Alkoholkonsumverbot", "Betteln aggressiv", "Grünanlage o.B.", "Keine Beanstandungen", "Urinieren", "Wilder Müll"])
...
with st.expander("➕ NEUEN EINSATZBERICHT ERSTELLEN", expanded=True):
    with st.form("bericht_form", clear_on_submit=True):
        ...
        with k_col1:
            kod_check = st.checkbox("🚓 KOD", value=True, disabled=True)
        ...
        with k_col2:
            funkstreife = ""
            if pol_check:
                funkstreife = st.text_input("🚔 Funkstreife / Dienststelle", placeholder="z.B. Augsburg 12/1")

        st.divider()
        st.subheader("📝 Sachverhalt")
        vorlage = st.selectbox("📑 Feststellung (Vorlage)", [None] + FESTSTELLUNGEN)
        # HIER: Bezeichnung von Berichtstext in Sachverhalt geändert
...
if os.path.exists(DATEI):
    ...
    # Filterung
    if suche:
        archiv_mask = archiv_data['Ort'].str.contains(suche, case=False) | archiv_data['AZ'].str.contains(suche, case=False)
        display_data = archiv_data[archiv_mask]
    else:
        display_data = archiv_data

    for i, r in display_data.iloc[::-1].iterrows():
        # Entschlüsseln nur für die Anzeige
        ...
        with st.expander("📝 Details & Sachverhalt"):
            ...
            # LÖSCH-BUTTON (Nur für Admins sichtbar)
            if st.session_state["admin_auth"]:
                if st.button(f"🗑️ Bericht {r['AZ']} unwiderruflich löschen", key=f"del_{i}"):
...
with st.sidebar:
    ...
    if st.checkbox("🔑 Admin-Modus (Löschen freischalten)"):
        pw_input = st.text_input("Admin-Passwort", type="password")
        if pw_input == ADMIN_PW:
            st.session_state["admin_auth"] = True
            st.success("Löschfunktion aktiv")
        elif pw_input != "":
